...
export function RepresentativeDashboardView({ userId, region, initialReports }: { userId: string, region: string, initialReports: any[] }) {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'draft': return <span className="px-2 py-1 rounded-full bg-bg-secondary text-text-secondary text-xs font-medium border border-border-medium">በመሰራት ላይ (Draft)</span>;
      default: return null;
    }
  };
            ...
            <div className="bg-bg-primary rounded-2xl border border-border-light shadow-sm overflow-hidden p-6">
              ...
              <div className="space-y-4">
                {reports.filter(r => r.admin_feedback).length === 0 ? (
                  <p className="text-sm text-text-muted text-center py-4 bg-bg-secondary/50 rounded-xl border border-dashed border-border-medium">
                    ምንም ግብረ መልስ አልተሰጠም
                  </p>
                ) : (
