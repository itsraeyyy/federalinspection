...
import { Admin, ALL_MODULES } from "@/types";
  ...
  useEffect(() => {
    adminService.getAdminById(id).then(data => {
      if (data) {
        ...
        if (data.role === 'committee_leader') {
          setSpecificRole('committee_leader');
          setValue('specificRoleType', 'committee_leader');
        } else if (data.modules?.includes('complaints') && data.modules?.length === 1) {
          setSpecificRole('complaint_receiver');
          setValue('specificRoleType', 'complaint_receiver');
        } else if (data.modules?.includes('news') && data.modules?.includes('documents') && data.modules?.length === 2) {
          setSpecificRole('content_manager');
          setValue('specificRoleType', 'content_manager');
        } else if (data.modules?.includes('assessment') && data.modules?.includes('personnel')) {
          setSpecificRole('assessment_coordinator');
          setValue('specificRoleType', 'assessment_coordinator');
        } else if (data.modules?.includes('forms')) {
          setSpecificRole('reports_officer');
          setValue('specificRoleType', 'reports_officer');
        } else if (data.modules?.includes('sletena')) {
          setSpecificRole('training_coordinator');
          setValue('specificRoleType', 'training_coordinator');
        } else {
          setSpecificRole('custom');
          setValue('specificRoleType', 'custom');
        }
      }
      setLoading(false);
    });
    ...
    try {
      ...
      if (data.accessLevel === 'all') {
        ...
        if (specificRole === 'committee_leader') {
          payload.role = 'committee_leader';
          payload.modules = ['complaints', 'committee-leader', 'abetuta', 'tikoma'];
        } else if (specificRole === 'complaint_receiver') {
          payload.role = 'admin';
          payload.modules = ['complaints'];
        } else if (specificRole === 'content_manager') {
          payload.role = 'admin';
          payload.modules = ['news', 'documents'];
        } else if (specificRole === 'assessment_coordinator') {
          payload.role = 'admin';
          payload.modules = ['assessment', 'personnel', 'statistics'];
        } else if (specificRole === 'reports_officer') {
          payload.role = 'admin';
          payload.modules = ['forms', 'admin_forms', 'statistics', 'map'];
        } else if (specificRole === 'training_coordinator') {
          payload.role = 'admin';
          payload.modules = ['sletena', 'feedback', 'qr-access'];
        } else if (specificRole === 'personnel_manager') {
          payload.role = 'admin';
          payload.modules = ['personnel', 'qr-access'];
        } else {
          payload.role = 'admin';
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8 h-full max-w-4xl mx-auto pb-10">
        ...
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={handleDelete} className="flex items-center gap-2 bg-danger/10 hover:bg-danger/20 text-danger px-5 py-2.5 rounded-full text-sm font-semibold transition-colors border border-danger/20 cursor-pointer">
        ...
        <div className="bg-surface-primary/40 rounded-[2rem] border border-border/30 p-7 md:p-8 backdrop-blur-md flex flex-col gap-6 shadow-sm">
          <div className="flex items-center gap-4 pb-2">
            <div className="w-14 h-14 rounded-2xl bg-brand-blue/10 border border-brand-blue/20 flex items-center justify-center font-bold text-brand-blue text-xl shadow-xs">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <input {...register('name')} type="text" className="w-full bg-surface-primary border border-border/50 rounded-xl p-3.5 text-sm text-text-primary focus:outline-none focus:border-brand-blue/50 transition-colors" />
                {errors.name && <span className="text-xs text-danger">{errors.name.message}</span>}
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-text-secondary uppercase tracking-widest">ኢሜይል</label>
                <input {...register('email')} type="email" className="w-full bg-surface-primary border border-border/50 rounded-xl p-3.5 text-sm text-text-primary focus:outline-none focus:border-brand-blue/50 transition-colors" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <input {...register('phone')} type="tel" className="w-full bg-surface-primary border border-border/50 rounded-xl p-3.5 text-sm text-text-primary focus:outline-none focus:border-brand-blue/50 transition-colors" />
